...
export class Generated1756221284484 implements MigrationInterface {
  name = 'Generated1756221284484';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`inventories\` (\`id\` int NOT NULL AUTO_INCREMENT, \`product_id\` int NULL, \`stock_type\` enum ('order', 'pre-order', 'sales', 'cancel') NOT NULL, \`status_delivery\` enum ('pending', 'delivered', 'dropped', 'completed', 'cancel') NOT NULL, \`status_payment\` enum ('paid', 'unpaid') NOT NULL, \`stock\` int NOT NULL DEFAULT '0', \`total_order\` int NOT NULL DEFAULT '0', \`sales\` decimal(10,2) NOT NULL DEFAULT '0.00', \`purchase\` decimal(10,2) NOT NULL DEFAULT '0.00', \`supplier_id\` int NULL, \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`inventories\` ADD CONSTRAINT \`FK_92fc0c77bab4a656b9619322c62\` FOREIGN KEY (\`product_id\`) REFERENCES \`products\`(\`id\`) ON DELETE NO ACTION ON UPDATE CASCADE`,
    );
    await queryRunner.query(
      `ALTER TABLE \`inventories\` ADD CONSTRAINT \`FK_3bed26289926c1ad4a08d4c7285\` FOREIGN KEY (\`supplier_id\`) REFERENCES \`suppliers\`(\`id\`) ON DELETE NO ACTION ON UPDATE CASCADE`,
    );
    await queryRunner.query(
